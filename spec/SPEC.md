...
### 1.4.1 DVE vs dxe-server 責務マトリクス（図）

```mermaid
graph TB
    subgraph DVE["DVE — Decision Visualization Engine (localhost:4174)"]
        DVE_DRE["DRE ステート検出<br/>FRESH / INSTALLED / OUTDATED / CUSTOMIZED"]
        DVE_DGE["DGE Gap ライフサイクル<br/>Active / Void / Archived"]
        DVE_SCAN["POST /api/scan<br/>マルチプロジェクト発見"]
        DVE_GRAPH["GET /api/graph<br/>Session→Gap→DD→Spec グラフ"]
        DVE_UI["Preact + Cytoscape UI<br/>決定グラフ可視化"]
    end

    subgraph DXE["dxe-server — DDE Visualization (localhost:4175)"]
        DXE_STATUS["GET /api/dde/status<br/>DDE 補完率ダッシュボード"]
        DXE_REPORT["GET /api/dde/weekly-report<br/>週次 Markdown / HTML レポート"]
        DXE_TREND["GET /api/dde/trend<br/>補完率の時系列推移"]
        DXE_REG["POST /api/projects/register<br/>DDE プロジェクト登録"]
        DXE_UI["ミニマル補完状態 UI<br/>バーグラフ・折れ線グラフ"]
    end

    subgraph SHARED["共有データソース"]
        CJ["completion.json<br/>(DDE 出力・計画)"]
        PJ["projects.json<br/>(dxe-server レジストリ)"]
    end
    ...
    style DVE fill:#e8f4f8,stroke:#2196F3
    style DXE fill:#f0f8e8,stroke:#4CAF50
    style SHARED fill:#fff8e1,stroke:#FFC107
```
...
#### 4.1.1 プロセスライフサイクル（Mermaid stateDiagram）

```mermaid
stateDiagram-v2
    [*] --> STARTING : npx dxe-server start

    STARTING --> RUNNING : 設定読み込み・ポートバインド成功
    STARTING --> ERROR : ポートバインド失敗 (EADDRINUSE 等)

    RUNNING --> STOPPING : Ctrl+C / SIGTERM
    RUNNING --> ERROR : 予期しない例外

    STOPPING --> [*] : 接続クローズ・ファイルフラッシュ完了
    ERROR --> [*] : エラーログ出力後に終了

    state RUNNING {
        [*] --> IDLE
        IDLE --> HANDLING : HTTPリクエスト受信
        HANDLING --> IDLE : レスポンス送信完了
        IDLE --> REPORTING : 週次レポートジョブ起動 (Option B)
        REPORTING --> IDLE : レポート生成完了
    }

    note right of STARTING
        Option C (ワンショット) は
        STARTING → GENERATING → DONE
        の3ステートのみ
    end note
```
...
#### C.4.1 ADR-002 判断フローチャート（Mermaid）
...
flowchart TD
    ...
    style OC fill:#cce5ff,stroke:#004499
    style ALT fill:#fff3cd,stroke:#856404
    style END_A fill:#ffcccc,stroke:#cc0000
    style END_B fill:#ccffcc,stroke:#006600
    style END_C fill:#cce5ff,stroke:#004499
```
